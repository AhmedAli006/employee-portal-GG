...
const PopUpComponent = ({ show, setShow, selectedEmployee, onSave, department }) => {
  ...
  const handleSubmit = async () => {
    ...
    try {
      await onSave(employee, !!selectedEmployee);
      setShow(false); // Close modal after successful API call
    } catch (error) {
  ...
  return (
    <Dialog open={show} onClose={() => setShow(false)} fullWidth maxWidth="sm">
      ...
      <DialogContent>
        ...
        <Select fullWidth name="departmentId" value={employee.departmentId} onChange={handleChange}>
          {department.map((dept) => (
        ...
        <FormControlLabel control={<Switch checked={employee.isActive} onChange={handleToggle} />} label="Active" />
      </DialogContent>
      <DialogActions>
